...
- 🎯 Entry points to execute your program using `python -m <pythontasks>` or `$ pythontasks` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/admondtamang/pythonTasks/branch/main/graph/badge.svg?token=pythonTasks_token_here)](https://codecov.io/gh/admondtamang/pythonTasks)
[![CI](https://github.com/admondtamang/pythonTasks/actions/workflows/main.yml/badge.svg)](https://github.com/admondtamang/pythonTasks/actions/workflows/main.yml)

Awesome pythontasks created by admondtamang

## Install

from source
```bash
git clone https://github.com/admondtamang/pythonTasks pythontasks
cd pythontasks
make install
```
...
```bash
pip install pythontasks
```

## Executing

```bash
$ pythontasks run --port 8080
```

or

```bash
python -m pythontasks run --port 8080
```

or

```bash
$ uvicorn pythontasks:app
```
...
```bash
❯ pythontasks --help
Usage: pythontasks [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ pythontasks create-user --help
Usage: pythontasks create-user [OPTIONS] USERNAME PASSWORD
...
```bash
pythontasks create-user admin admin --superuser
```
...
Run with `pythontasks run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `pythontasks create-user`.
...
-----------------------------------------------------
pythontasks/__init__.py              4      0   100%
pythontasks/app.py                  16      1    94%
pythontasks/cli.py                  21      0   100%
pythontasks/config.py                5      0   100%
pythontasks/db.py                   10      0   100%
pythontasks/models/__init__.py       0      0   100%
pythontasks/models/content.py       47      1    98%
pythontasks/routes/__init__.py      11      0   100%
pythontasks/routes/content.py       52     25    52%
pythontasks/routes/security.py      15      1    93%
pythontasks/routes/user.py          52     26    50%
pythontasks/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in pythontasks/default.toml.
